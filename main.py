...
@app.route("/about")        # Similarly when we reach this end point()
def about():
    return render_template('about.html', params=params)


@app.route("/dashboard", methods=['GET', 'POST'])        # Similarly when we reach this end point()
def dashboard():
    ...
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            if sno=='0':
                ...
                post.date = date
                db.session.commit()
                return redirect('/dashboard')                       # Here i have changed initially it was '/edit/'+sno
        post = Posts.query.filter_by(sno=sno).first()
